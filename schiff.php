<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" lang="de" prefix="og: http://ogp.me/ns#">
    <head>
        <title>Das Schiff — Jugend Rettet</title>
        <meta name="description" content="Wir sind eine Organisation von jungen Erwachsenen für junge Erwachsene. Wir sammeln Gelder für den Erwerb eines Schiffes und mit diesem Schiff werden wir private Seenotrettung auf dem Mittelmeer betreiben." />
        <meta charset="utf-8">
        <meta name="author" content="Jugend Rettet" />
        <meta name="keywords" content="Jugend, Rettet, Jugend Rettet, Jugend Rettet e. V., Seenotrettung, Mittelmeer, Flüchtlinge, Spenden, Europa, Organisation" />
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">

        <meta property="og:title" content="Jugend Rettet" />
        <meta property="og:description" content="Wir sind eine Organisation von jungen Erwachsenen für junge Erwachsene. Wir sammeln Gelder für den Erwerb eines Schiffes und mit diesem Schiff werden wir private Seenotrettung auf dem Mittelmeer betreiben." />
        <meta property="og:url" content="https://jugendrettet.org/schiff" />
        <meta property="og:image" content="http://jugendrettet.org/images/jr-banner.jpg" />
        <meta property="og:image:secure_url" content="https://jugendrettet.org/images/jr-banner.jpg" />
        <meta property="og:type" content="website" />
        <meta property="og:site_name" content="Jugend Rettet" />

        <link rel="alternate" hreflang="en" href="http://en.jugendrettet.org/schiff" />

        <link rel="shortcut icon" type="image/x-icon" href="./graphics/jr.ico" />
        <link rel="mask-icon" href="./graphics/jr-mask.svg" color="rgb(40,100,130)">
        <link rel="apple-touch-icon" href="./graphics/apple-touch-icon.png"/>
        <link rel="stylesheet" href="./css/waves.1.css" type="text/css" />

        <link rel="stylesheet" href="./css/general.css" type="text/css" />
        <link rel="stylesheet" href="./css/main.css?9" type="text/css" />
        <link rel="stylesheet" href="./css/desktop.css?5" media="screen and (min-width: 851px)" type="text/css" />
        <link rel="stylesheet" href="./css/mobile.css?6" media="screen and (max-width: 850px)" type="text/css" />

        <script src="./js/jquery-1.10.2.min.js"></script>
        <script type="text/javascript">
            $(window).load(function() {
            });
        </script>
        <style type="text/css">
            #was {
                background-image: url(./images/06-2016/heck.jpg);
            }
            #warum {
                background-image: url(./images/06-2016/andeck.jpg);
            }
            #wie {
                background-image: url(./images/06-2016/dock.jpg);
            }
        </style>
    </head>

    <body>
        <div id="pop-back"></div>
        <div id="header-back"></div>
	    <div id="wrap">
            <header>
                <a id="logo-wrap" href="/">
                    <img id="logo" src="./graphics/logo.svg" alt="Jugend Rettet">
                    <div id="title">Jugend <b>Rettet</b></div>
                </a>
                <div id="bread">
                    <div class="stroke" id="br-top-stroke"></div>
                    <div class="stroke" id="br-bottom-stroke"></div>
                </div>
                <div id="popup-close">
                    <div id="cross">
                        <div class="stroke rotate-open-top rotate-close-top" id="cr-top-stroke"></div>
                        <div class="stroke rotate-open-bottom rotate-close-bottom" id="cr-bottom-stroke"></div>
                    </div>
                </div>
                <nav>
                    <ul class="hidden">
                        <li><a href="news">News</a></li>
                        <li><a href="schiff" class="indicator">Das Schiff</a></li>
                        <li><a href="team">Team</a></li>
                        <li><a href="retten">Retten</a></li>
                        <li><a href="spenden">Spenden</a></li>
                        <li><a href="europe">Jugend Rettet <span class="lowercase">europe</span></a></li>
                    </ul>
                </nav>
            </header>
            <div id="content">
                <div id="menu" class="triple">
                    <div class="child" id="was">
                        <div class="show" id="was-btn" style="height: 100%;">
                            <div class="tint">
                                <div class="helper">
                                    <h3>Was?</h3>
                                    <hr class="narrow">
                                    <div class="description">
                                        <p>Das macht <i>Jugend Rettet</i> genau.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="child" id="warum">
                        <div class="show" id="warum-btn" style="height: 100%;">
                            <div class="tint">
                                <div class="helper">
                                    <h3>Warum?</h3>
                                    <hr class="narrow">
                                    <div class="description">
                                        <p>Das sind unsere Motive.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="child" id="wie">
                        <div class="show" id="wie-btn" style="height: 100%;">
                            <div class="tint">
                                <div class="helper">
                                    <h3>Wie?</h3>
                                    <hr class="narrow">
                                    <div class="description">
                                        <p>Das tun wir, um Menschen auf dem Mittelmeer vor dem Ertrinken zu retten.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <footer>
                    <div id="peek-container"></div>
                    <nav class="footer">
                        <ul>
                            <li><a href="faq">FAQ</a></li>
                            <li class="likelink"><span class="peek" id="folgen">Folgen</span></li>
                            <li><a href="presse">Presse</a></li>
                            <li><a href="impressum">Impressum</a></li>
                            <li><a href="transparenz">Transparenz</a></li>
                            <li class="br"></li>
                            <li><a href="https://en.jugendrettet.org/schiff">en</a></li>
                        </ul>
                    </nav>
                </footer>
            </div>
            <div class="popup" id="was-btn-pop">
                <div class="single">
                    <h3>Was?</h3>
                </div>
                <div class="container single">
                    <div class="column">
                        <p>Wir sind eine Organisation von jungen Erwachsenen, die Gelder für den zur Seenotrettung auf dem Mittelmeer sammeln. Mit unserem Schiff, der IUVENTA, betreiben wir ab Juli 2016 private Seenotrettung auf dem Mittelmeer. Wir lokalisieren Menschen in Seenot und werden sie, je nach Situation, entweder durch Rettungsinseln sichern oder an Bord nehmen. Unser Ziel ist klar: weniger Tote im Mittelmeer. Darüber hinaus werden wir mit <a href="europe">Jugend Rettet europe</a> eine Diskussionsplattform über die europäische Asylpolitik für junge Erwachsene schaffen. Wir glauben an das Potenzial unserer jungen Generation, die aktuelle Situation durch eine breite asylpolitische Diskussion in ganz Europa zu verbessern.</p>
                        <h4>Transparenz</h4>
                        <a href="./transparenz"><img style="width:calc(100% - 12px);padding:5px;background:white;border:solid 1px #286482;" src="./graphics/Transparente_Zivilgesellschaft.svg"></a>
                        <br>
                    </div>
                </div>
            </div>
            <div class="popup" id="warum-btn-pop">
                <div class="triple">
                    <h3>Warum?</h3>
                </div>
                <div class="container triple">
                    <div class="column mehr" id="gründe">
                        <h4><img class="icon" src="./graphics/gru.svg" alt="Gründe">Gründe</h4>
                        <p>Warum haben wir <i>Jugend Rettet</i> gegründet?</p>
                        <p>Wir haben die zwei Vorsitzenden von <i>Jugend Rettet</i> und Menschen, die über das Mittelmeer geflüchtet sind diese Frage beantworten lassen.</p>
                        <div class="mehr-klappe"><br>
                            <!--

                            <div class="popup-img" style="background-image: url('./images/news.jpg');"></div>

                            -->
                            <img class="portrait" src="./images/07-2016/Jakob_Lena_Jugend_Rettet.jpg" alt="Jakob und Lena Jugend Rettet">
                            <p>Lena: „Ich engagiere mich seit langem in einem Flüchtlingsheim in dem viele Menschen leben, die über das Mittelmeer geflüchtet sind. In mir entstand der Wunsch, mich für die Menschen auf dem Mittelmeer einzusetzen. Bei <i>Jugend Rettet</i> habe ich ein konkretes Aktionsfeld gefunden, um mich konstruktiv für meine politischen Ideale einzusetzen.“
                            </p><br>
                            <!--

                            <div class="popup-img" style="background-image: url('./images/neu.jpg');"></div>

                            -->

                            <p>Jakob: „Ich kann die täglichen Meldungen über neue Schiffsunglücke nicht mehr stillschweigend hinnehmen. <i>Jugend Rettet</i> hat als Zusammenschluss junger Leute das Potenzial etwas zu verändern. Europaweit vernetzen wir motivierte Jugendliche und zeigen, wie wichtig es ist, einen gemeinsamen, europäischen Entwurf für eine humanere Asylpolitik zu finden. Unser Schiff ist keine langfristige Lösung, aber es wird Menschenleben retten und es wird die Frage aufwerfen, warum Jugendliche anstelle der europäischen Regierungen diese Aufgabe in privater Initiative übernehmen müssen.“</p><br>
                            <p>Mohammad, 24 aus Homs, Syrien: „Am 15. März 2011 begann die Revolution in meiner Stadt. Zuerst wurde die Universität geschlossen. Ich habe Bauwesen studiert. Am 10. März 2012 flüchtete ich nach Ägypten, um nicht von der syrischen Armee eingezogen zu werden. Ich wusste, wenn ich da hin muss, werde ich Menschen töten. Das wollte ich nicht. Doch das Leben in Ägypten ist schwer. Man arbeitet immer für einen sehr kleinen Lohn. Man kämpft jeden Tag für das Überleben und weiß, dass es hier nie besser wird. Es ist schwer für die Syrer und auch die Ägypter. Das Boot nach Europa war mein einziger Ausweg. Ich hatte keine andere Chance.“</p>
                        </div>
                        <a class="mehr-btn">Mehr ...</a>
                    </div>
                    <div class="column mehr" id="sit">
                        <h4><img class="icon" src="./graphics/sit.svg" alt="Aktuelle Situation">Aktuelle Situation</h4>
                        <p>218.000 Menschen sind 2014 über das Mittelmeer geflüchtet. 3500 von ihnen haben es nicht geschafft.</p>
                        <div class="mehr-klappe"><br>
                            <p>In der Europäischen Union gilt das Grundrecht auf Asyl, allerdings gibt es kaum noch Wege Asyl legal zu beantragen, wodurch die Menschen gefährliche Fluchtrouten wie die über das Mittelmeer auf sich nehmen. (<a href="http://www.zeit.de/politik/2016-04/fluechtlingsrouten-europa-mittelmeer" target="_blank">ZEIT</a>, April 2016)</p><br>
                            <p>2015 kamen eine Million Menschen über das Mittelmeer in seeuntauglichen Booten nach Europa (<a href="https://www.uno-fluechtlingshilfe.de/nothilfe/europa.html" target="_blank">Angaben der UNO</a>, Mai 2016). Bei der gefährlichen Überfahrt sind 2015 3771 Menschen gestorben oder verschwunden, laut einer Schätzung des UN-Flüchtlingshilfswerks.</p><br>
                            <p>
                              2016 sind bereits 187.920 Flüchtlinge und Migranten auf dem Seeweg in Europa angekommen - davon 155.765 Personen in Griechenland und 31.252 in Italien (<a href="https://www.uno-fluechtlingshilfe.de/news/italien-1000-menschen-aus-seenot-gerettet-475.html" target="_blank">Angaben der UNO</a>, Mai 2016)
                            </p><br>
                            <p>
                              60% aller Jugendlichen wären bereit sich mit Gleichgesinnten für ihre politischen Interessen einzusetzen, wenn sie das Gefühl hätten, etwas verändern zu können (<a href="http://www.dgb.de/themen/++co++7525a922-220a-11df-7d31-00093d10fae2" target="_blank">DGB</a>, September 2009). Jeder 4. Jugendliche würde sich bei konkreten Projekten in der Stadt oder Gemeinde engagieren (<a href="http://www.spiegel.de/schulspiegel/leben/kinderhilfswerk-umfrage-kinder-wuerden-sich-gern-staerker-engagieren-a-890245.html" target="_blank">SPIEGEL Online</a>, März 2013). Aktuell gibt es keine vergleichbare von Jugendlichen initiierte Organisation mit dem Ziel der privaten Seenotrettung auf dem Mittelmeer. Konkrete, unabhängige Aktionsfelder für junge Erwachsene fehlen häufig, wodurch der Wille, sich politisch zu engagieren, oft nicht umgesetzt werden kann.
                            </p>
                        </div>
                        <a class="mehr-btn">Mehr ...</a>
                    </div>
                    <div class="column mehr" id="ide">
                        <h4><img class="icon" src="./graphics/ide.svg" alt="Ideal">Ideal</h4>
                        <p>Wir wollen ein Ende der Gleichgültigkeit der Europäischen Gemeinschaft gegenüber dem Sterben auf dem Mittelmeer.</p>
                        <div class="mehr-klappe"><br>
                            <p>Wir setzen uns für eine humanere, gerechtere und engagiertere Asylpolitik ein.</p>
                            <p>Menschen auf der Flucht unterscheiden sich von uns nur durch die Notsituation, in die sie unverschuldet geraten sind.  Ihr Recht auf Menschenwürde gilt für sie uneingeschränkt, auch während der Flucht. Um das zu erreichen, wollen und müssen wir Verantwortung übernehmen.</p>
                            <p>Junge Erwachsene sollen ihr Potenzial nutzen und sich politisch engagieren. Jeder kann etwas ändern. Als moderne Europäer vertrauen wir dabei in die Kraft eines vereinten europäischen Engagements, um gemeinsam für mehr Menschlichkeit in der Asylpolitik zu sorgen.</p>
                        </div>
                        <a class="mehr-btn">Mehr ...</a>
                    </div>
                </div>
            </div>
            <div class="popup" id="wie-btn-pop">
                <div class="single">
                    <h3>Wie?</h3>
                </div>
                <div class="container single">
                    <div class="column newsacc">
                        <h4>Das Schiff</h4>
                        <script type="text/javascript">
                            var width="100%";
                            var height="500";
                            var zoom="10";
                            var mmsi="244870962";
                            var show_track=true;
                            var click_to_activate=false;
                            var default_maptype=1;
                        </script>
                        <script type="text/javascript"
                            src="https://www.vesselfinder.com/aismap.js">
                        </script>
                        <br>
                        <img src="./images/06-2016/IUVENTA1.jpg" alt="Jugend Rettet Rettungsschiff IUVENTA" style="max-width:100%">
                        <p>Im Mai 2016 haben wir unser Schiff gekauft, mit vielen Freiwilligen umgebaut und auf den Namen IUVENTA getauft.  Aus folgenden Gründen ist sie für Rettungsmissionen auf hoher See geeignet:</p>
                        <br>
                        <ul>
                          <li>&#10003; Die IUVENTA verfügt über eine große Fläche an Deck. So können wir, wenn es die Situation erfordert, 80-100 Menschen an Bord nehmen. Über die Deckfläche wird eine LKW-Plane gespannt sein, die die Menschen an Deck vor Sonneneinstrahlung oder Regen schützt. Im Winter wird diese Plane an den Seiten nach unten gespannt sein, wodurch ein geschlossener Innenbereich entsteht.</li>
                        </ul>
                        <img src="./images/06-2016/IUVENTA2.jpg" alt="Jugend Rettet Rettungsschiff IUVENTA" style="max-width:100%">
                        <br>
                        <ul>
                          <li>&#10003; Die IUVENTA zeichnet sich durch eine niedrige Bordwand aus. So können wir Menschen leicht und flexibel an Bord nehmen und auch unsere Beiboote werden bei einer niedrigen Bordwand leichter ins Wasser zu heben sein.</li>
                          <li>&#10003; Sie hat einen Hydraulikkran, der die Beiboote schnell ins Wasser heben kann. Wir haben zwei Beiboote an Bord. Bei einer Rettungsmission bleibt die IUVENTA in einiger Entfernung. Mit den Beibooten nähern wir uns von hinten links und rechts ran. Ein Beiboot bleibt immer vor Ort, um mit den Menschen zu kommunizieren. Das andere shuttelt die Menschen zu IUVENTA. So wird bei einer Rettung Panik vermieden.</li>
                          <li>&#10003; Unser Schiff hat Platz für ausreichend Frischwassertanks. Einige Menschen in Not müssen mehrere Tage ohne Wasser auf ihrem Boot verbringen. Um die Dehydrierung zu lindern, werden die Menschen mit Wasser versorgt.</li>
                        </ul>
                        <img src="./images/06-2016/IUVENTA3.jpg" alt="Jugend Rettet Rettungsschiff IUVENTA" style="max-width:100%"><br>
                    </div>
                </div>
            </div>
        </div>
        <!-- Scripts -->
        <script src="./js/main.js?0"></script>
        <script src="./js/peekfield.js?2"></script>
    </body>
</html>
